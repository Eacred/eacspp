...
import (
	...
	"github.com/Eacred/eacspp/dcnet"
	"github.com/Eacred/eacspp/messages"
	"github.com/Eacred/eacspp/x25519"
...
func (s *Session) run(ctx context.Context, n int, br *messages.BR) error {
	...
	if n == 0 || s.freshGen {
		...
		if len(s.dcMsg) != s.mcount {
			return errors.New("eacspp: Gen returned wrong message count")
		}
		for _, m := range s.dcMsg {
			if len(m) != MessageSize {
				return errors.New("eacspp: Gen returned bad message length")
			}
		}
	}
