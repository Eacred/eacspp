...
/*
Package wire implements the Eacred wire protocol.

For the complete details of the Eacred protocol, see the official wiki entry
...
supported Eacred messages to and from the wire.  This package does not deal
with the specifics of message handling such as what to do when a message is
received.  This provides the caller with a high level of flexibility.

Eacred Message Overview

The Eacred protocol consists of exchanging messages between peers.  Each
message is preceded by a header which identifies information about it such as
which Eacred network it is a part of, its type, how big it is, and a checksum
...
of the currently supported Eacred messages are provided.  For these supported
messages, all of the details of marshalling and unmarshalling to and from the
wire using Eacred encoding are handled so the caller doesn't have to concern
themselves with the specifics.

Message Interaction

The following provides a quick summary of how the Eacred messages are intended
to interact with one another.  As stated above, these interactions are not
directly handled by this package.  For more in-depth details about the
appropriate interactions, see the official Eacred protocol wiki entry at
...
and write Eacred messages.  The following sections provide a quick overview of
...
The Eacred network is a magic number which is used to identify the start of a
message and which Eacred network the message applies to.  This package provides
...
As discussed in the Eacred message overview section, this package reads
and writes Eacred messages using a generic interface named Message.  In
...
In order to unmarshall Eacred messages from the wire, use the ReadMessage
function.  It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a Eacred peer.  Example syntax is:

	// Reads and validates the next Eacred message from conn using the
	// protocol version pver and the Eacred network btcnet.  The returns
...
In order to marshall Eacred messages to the wire, use the WriteMessage
function.  It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a Eacred peer.  Example syntax to request addresses
from a remote peer is:
	...
	// pver, and the Eacred network btcnet.  The return is a possible
	// error.
