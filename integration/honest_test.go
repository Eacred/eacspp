...
import (
	...
	"github.com/Eacred/eacspp/internal/nettest"
	"github.com/Eacred/eacspp/server"
...
func newConfirmer(input *wire.TxIn, change *wire.TxOut) eacspp.GenConfirmer {
	return &confirmer{
		tx: &wire.MsgTx{
			Version: 1,
...
func (c *confirmer) Gen() ([][]byte, error) {
	ms := make([][]byte, *mFlag)
	for i := range ms {
		ms[i] = make([]byte, eacspp.MessageSize)
...
func TestHonest(t *testing.T) {
	...
	svr, err := server.New(eacspp.MessageSize, newCoinJoin, *epochFlag)
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := range peers {
		...
		ses, err := eacspp.NewSession(rand.Reader, logger, commitment, *mFlag)
		if err != nil {
			t.Fatal(err)
		}
		peers[i].ses = ses
